/*
  CHAPTER 48 — وضع قريباً لقسم الأدلة
  لإعادة القسم كما كان: احذف سطر تحميل هذا الملف من index.html.
*/
(function () {
  function showComingSoon() {
    const library = document.getElementById("library");
    if (!library || document.getElementById("guides-coming-soon-notice")) return;

    const firstGuideLink = library.querySelector('a[href*="drive.google.com"]');
    const guidesGrid = firstGuideLink?.closest(".grid");
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", showComingSoon, { once: true });
  } else {
    showComingSoon();
  }
})();
